...
/**
 * Open the walk-in booking modal.
 *
 * params: { hotelId, roomId, room, initialDate, onSuccess }
 *   - initialDate: ISO day; pre-selected as check_in
...
export async function openWalkinModal({ hotelId, roomId, room, initialDate, onSuccess }) {
  ...
  // Fetch availability so we know which days to disable.
  ...
  const dayState = {}; // iso → { blocked, booked }
  ...
  let checkOut = null; // optional second click
  ...
  function renderCal() {
    ...
    for (let i = 0; i < DAYS_AHEAD; i++) {
      const d = todayPlus(i);
      const st = dayState[d]; // undefined | "blocked" | "booked"
      const dt = new Date(d);
  ...
  function pickDay(d) {
    // 1st click: set check_in, clear check_out
    // 2nd click on a later day: set check_out (exclusive boundary = d+1)
    // 2nd click on same day or earlier: reset to check_in only
    if (!checkOut && d > checkIn && !rangeHasBlock(checkIn, d)) {
      checkOut = addDays(d, 1); // d itself is the last night → +1 exclusive
    } else {
      checkIn = d;
      checkOut = null;
    }
  ...
  async function doLookup(form) {
    ...
    try {
      ...
      if (!c) return;
      // pre-fill empty fields only — don't overwrite what partner typed
