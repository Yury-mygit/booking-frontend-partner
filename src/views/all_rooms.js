...
// Columns we filter on. `key` is the data field; `display` is what the filter
// matches against (e.g. for "status" we match against the localized label so
// the user can type what they see, not the enum name).
const COLS = ["hotel", "room", "status"];

let _rows = [];        // last fetched data
let _filters = {       // current text in each filter input
  hotel: "",
  room: "",
  status: "",
};
...
function renderRows() {
  ...
  // Attach long-press / right-click handlers on filterable cells.
...
function rowHtml(r) {
  ...
  return `
    <tr>
      ...
      <td><a class="hotel-edit-btn" href="#/room/${r.hotel_id}/${r.room_id}/availability" title="${t("rooms.col_calendar")}" aria-label="${t("rooms.col_calendar")}">📅</a></td>
    </tr>`;
}
...
function parseTokens(raw) {
  // Split on commas, trim, drop empties. Group into positives / negatives.
  const positives = [];
  const negatives = [];
...
function showContextMenu(x, y, td) {
  ...
  // Cell's "raw" value: visible text content of the cell, but for the status
  // cell that's a pill — use textContent which still gives us the label.
