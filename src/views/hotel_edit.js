...
import { navigate, setBottomNav, setPageTitle } from "../router.js";
...
function renderChecklistItem(c) {
  ...
  if (!c.ok && c.action) {
    ...
    if (c.action.tab) {
      ...
      action = `<a href="#/room/${hotelId}/${c.action.room_id}" class="check-action">${fixLabel}</a>`;
    } else if (c.action.nav === "rooms") {
      action = `<a href="#/hotel/${hotelId}/rooms" class="check-action">${fixLabel}</a>`;
    }
  }
  return `
